...
bool RR_API::getImage(char *name, unsigned char *pixels, int *width, int *height, unsigned int *max, char *mode)
{
	...
	if ((stricmp(mode, "RGB")==0)||(stricmp(mode, "GRAY")==0)||(stricmp(mode, "BINARY")==0)||(stricmp(mode, "RED")==0)||(stricmp(mode, "GREEN")==0)||(stricmp(mode, "BLUE")==0)||(stricmp(mode, "JPEG")==0))
		format=mode;
	else
		format="RGB";
...
int RR_API::moduleTimer(char *module, int count)
{
  ...
  if (readMessage(handle, (unsigned char *)buffer, MAX_BUFFER_SIZE)>0)
	{
		if (sscanf_s(buffer, "<response><%*[^>]>%[^<]</%*[^>]></response>", result, 32)>=1)
		{
			return atol(result);
		}
		else
