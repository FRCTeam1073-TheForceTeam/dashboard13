...
int main(int argc, char *argv[])
{
	...
	// the routines below are shown as examples and do not perform a specific task. You can use
	// this as a reference on how to make the appropriate calls.
	...
	unsigned int max = 320*240*3;
	// get a JPEG version of the processed image and save that to disk
	if (rr.getImage("processed", image, &width, &height, &max, "JPEG"))
	{
		FILE *fp=fopen("c:\\temp\\test.jpg", "w+b");
		if (fp)
		{
			fwrite(image, max, 1, fp);
			fclose(fp);
		}
	}
...
/*
	...
	if (rr.getImage(image, &width, &height, &max))
	{
	...
	rr.loadPPM("c:\\www\\RoboRealm\\bin\\remo.ppm", image, &width, &height, 320*240*3);
	...
	// delete a module
	rr.toggleModule("RGB_Filter", 0);
	rr.deleteModule("RGB_Filter", 0);

	// determine module cpu time
	int cpuTime = rr.moduleTimer("RGB_Filter");

*/

	// close the RoboRealm application (as in force it to terminate) .. if you want too ... otherwise leave it running
	//rr.close();
